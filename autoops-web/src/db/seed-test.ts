...
// Prefixed with TEST- so they cannot collide with faker.vehicle.vrm() values
// generated by the dev seed (seed.ts), which would otherwise leave dev orders
// holding FK references to these vehicles and block the test seed.
const TEST_VEHICLE_PLATES = ['TEST-CB-1234', 'TEST-PA-9999'];
const TEST_VEHICLE_DESCRIPTIONS = [
  'Test Vehicle — Red Toyota',
  'Test Vehicle — Blue VW',
  'Test Vehicle — Updated',
];
...
// Prefixed/suffixed so seed-orders.ts (which picks random non-Unknown clients)
// can't recreate the FK-collision scenario across seed cycles.
const TEST_CLIENT_NAMES = ['Test Suite Client (autoops-test)'];
